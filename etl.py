...
def process_song_data(spark, input_data, output_data):
    """ Load data from song_data and extract columns for songs and artist tables and load them to AWS S3
    
        Args:
             spark: Spark session
             input_data: song data for extracting columns
             output_data: extracted data columns
             
        Return None
    """
    print("started loading song data")
    ...
    .parquet(os.path.join(output_data, 'songs/songs.parguet'), 'overwrite')
    ...
    print("finished preprocessing  song data")

def process_log_data(spark, input_data, output_data):
    """ Load data from log_data and extract columns, then write data into parquet to be loaded in AWS S3
    
        Args:
             spark: Spark session
             input_data: long data for extracting columns
             output_data: extracted data columns
    ...
    songplays_table.write.partitionBy(
        'year', 'month').parquet(os.path.join(output_data,
                                 ...
                                 'overwrite')
    print("finished preprocessing log data")
